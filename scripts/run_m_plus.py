...
from src.ml import fit_xgb, predict_proba, HAS_XGB, HAS_CB
...
def main(n_sims: int = 50000, dc_since: str = "2014-01-01",
    ...
    train_feat = build_match_features(train, train, elo, dc=dc, fifa_rank=fifa_rank)
    train_feat["pi_diff"] = train_pi.values

    test_feat = build_match_features(train, test, elo, dc=dc, fifa_rank=fifa_rank)
    ...
    W_XGB = 1.0 - W_ELO
    print(f"  Grid search -> W_ELO={W_ELO:.2f}  W_XGB={W_XGB:.2f}")
    ...
    full_feat = build_match_features(df, df, elo_full, dc=dc_full, fifa_rank=fifa_rank)
    ...
    out_path = ROOT / "outputs" / "champion_probs_Mplus.csv"
    ...
    pd.DataFrame([meta]).to_csv(ROOT / "outputs" / "model_Mplus_meta.csv", index=False)
